...
export const LEAGUE_OWN_TEAM = 'CNPN';
...
export function crestFor(name) {
  return TEAM_CRESTS[name] || { initials: name.slice(0, 2).toUpperCase(), bg: 'linear-gradient(135deg,#8FA3C2,#5B6B7C)' };
}

export function ownPositionLabel() {
  const own = LEAGUE_STANDINGS.find((t) => t.team === LEAGUE_OWN_TEAM);
  return own ? `${own.pos}º` : '—';
}
